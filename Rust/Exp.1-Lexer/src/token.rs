...
fn reversed_words() -> HashMap<String, TokenType> {
...
impl Tokenizer {
    ...
    // C语言中的连行，用`\`连接的两行，被看做是一行
    // 然后填充换行符，使得总行数保持不变
    fn remove_backslash_newline(&mut self) {
        let mut pos = 0;
        let mut cnt = 0; // 统计连行个数
...
pub fn print_token_stream(tokens: &Vec<Token>) {
    for token in tokens {
        println!("{:?} @ {:?} -> {:?}", token.ty, token.start, token.end);
    }
}
